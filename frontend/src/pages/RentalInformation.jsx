...
import { 
  ...
  ArrowRight, ChevronDown, ShoppingCart, Phone, Mail, MapPin, Star, ShieldCheck, LogOut, Loader2
} from 'lucide-react';
...
function RentalInformation() {
  ...
  useEffect(() => {
    const fetchEquipmentById = async () => {
      try {
        ...
        const response = await fetch(`http://localhost:3000/rental/equipment/${id}`);
        ...
        setEquipment({
          ...
          nextAvailableDate: item.next_available_date || new Date().toISOString().split("T")[0],
          image: item.image_url || `https://placehold.co/600x400?text=${encodeURIComponent(item.name || "Equipment")}`,
        });
        ...
        const checkAvail = async () => {
          ...
          try {
            const res = await fetch(
              `http://localhost:3000/rental/check-availability/${id}?start_date=${startDate}&end_date=${endDate}`
            );
            const data = await res.json();
            if (!data.available) {
              setAvailabilityError(
                data.next_available_date
                  ? `محجوزة حتى ${new Date(data.next_available_date).toLocaleDateString('ar-EG', { year: 'numeric', month: 'long', day: 'numeric' })} — اختار تاريخ بعده`
                  : "المعدة غير متاحة في هذه الفترة"
              );
            }
          } catch (err) {
            setAvailabilityError("فشل التحقق من الإتاحة");
          } finally {
  ...
  const isFormValid = startDate !== '' && endDate !== '' && rentalTotal > 0 
  ...
  // ✅ لو مفيش token خد المستخدم للـ login
  if (!token) {
    navigate("/login");
    return;
  }
  ...
  try {
    ...
    if (!response.ok) {
      setAvailabilityError(data.error || "فشل الإضافة للسلة");
      return;
    }
    ...
    setAvailabilityError("حدث خطأ، حاول تاني");
  }
};
  ...
  if (error || !equipment) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center font-['Cairo'] text-right" dir="rtl">
        ...
        <button onClick={() => navigate(-1)} className="bg-[#1e2d3e] text-white px-8 py-3 rounded-2xl font-black hover:bg-[#d35400] transition">
          رجوع
        </button>
      </div>
    );
          ...
          <div className="relative">
            ...
            {isDropdownOpen && (
              <div className="absolute left-0 mt-2 w-48 bg-white rounded-xl shadow-xl border border-gray-100 py-2 z-50 text-right font-bold">
                ...
                <button onClick={() => navigate('/login')} className="w-full text-right px-4 py-2.5 text-sm text-red-600 hover:bg-red-50">تسجيل خروج</button>
              </div>
            )}
          </div>
        </div>
              ...
              <div className="bg-orange-50 text-orange-700 p-4 rounded-xl font-bold text-xs border border-orange-100 space-y-2">
                ...
                <p className="pr-7 opacity-80 underline italic text-[11px]">
                  متاحة للحجز بعد تاريخ: {equipment.nextAvailableDate 
                    ? new Date(equipment.nextAvailableDate).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })
                    : "غير محدد"}
                </p>
              </div>
            ) : (
              <div className="flex items-center gap-2 bg-white text-green-700 p-4 rounded-xl font-bold text-xs border border-green-100">
          ...
          <section className="bg-white rounded-3xl p-8 shadow-lg border border-gray-100">
            ...
            <form className="space-y-6">
              <div className="grid sm:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <input 
                    ...
                    className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl font-bold outline-none focus:border-[#d35400]" 
                  />
                </div>
                <div className="space-y-2">
              ...
              {checkingAvailability && (
                <div className="p-3 bg-blue-50 text-blue-600 rounded-xl text-xs font-bold flex gap-2 items-center">
                  <Loader2 size={14} className="animate-spin" /> جاري التحقق من الإتاحة...
                </div>
              )}
              ...
              <AnimatePresence>
                {days > 0 && availabilityError === '' && !checkingAvailability && (
                  <motion.div 
                    initial={{ opacity: 0, y: 10 }} 
                    animate={{ opacity: 1, y: 0 }} 
                    className="bg-[#1e2d3e] text-white p-6 rounded-2xl space-y-4 shadow-xl"
                  >
